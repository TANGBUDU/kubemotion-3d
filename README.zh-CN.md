...
- **14 节完整验证课程：** 从 `why-kubernetes-exists` 到 `hpa`，覆盖集群结构、Pod／Container、Namespace 与 Node、Deployment 所有权、清单、Pending 调度、重启与替换、Label／Selector、Service／EndpointSlice、DNS、外部 Gateway 流量及 HPA 扩容
- **Manifest 顺序：** `why-kubernetes-exists` → `cluster-overview` → `pod-and-container` → `pod-and-placement` → `deployment-replicaset-and-pods` → `manifest-to-running-pod` → `pending-and-scheduling` → `container-restart-vs-pod-replacement` → `labels-and-selectors` → `service-routes-to-pods` → `dns-and-service-discovery` → `probes-and-rolling-update` → `full-external-request` → `hpa`
- **从底盘逐层学习：** 期望状态 → 集群底盘 → Pod／Container → 逻辑作用域与物理放置 → 工作负载所有权 → API／调度 → 自愈 → 选择 → Service 流量 → DNS → 探针与滚动更新 → 外部流量 → 自动扩缩
...
- **8 条已验证 Flow Story：** 四条 P0 与四条 P1 故事基于完整课程历史编译，并持续显示可验证的因果路线
- **8 节规划中课程：** 只作为路线图显示，不视为已经完成
...
Service 选择另一个 Ready 后端，而不是迁移已经运行的请求。十四节课程都使用带证据与结论
...
视觉对象、按视图分离的布局、八条完整 Flow Story、桌面／移动导航、语言持久化，以及镜头、
路线和标签门禁。必需视觉截图覆盖 1440×900、1280×720 与 390×844；渲染资源还要通过十四节
