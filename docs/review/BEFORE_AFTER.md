...
| ReplicaSet            | API-like desired/current/ready counters obscured the real fields.      | Explicit `.spec.replicas`, `.status.replicas`, and `.status.readyReplicas`, rendered as `SPEC / OBSERVED / READY`.                      |
