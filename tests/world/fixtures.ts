...
export const IDS = {
  ...
  oldContainer: 'container-status:shop:Pod:api-a-old:Container:api',
  newPod: 'api-object:namespaced:shop:Pod:api-d-new',
  newContainer: 'container-status:shop:Pod:api-d-new:Container:api',
...
export const oldContainerEntity = (status: 'running' | 'terminated' = 'running'): WorldEntity => ({
  ...
  data: {
    ...
    state:
      status === 'running'
        ? { kind: 'running', startedAt: '2026-08-08T00:00:00Z' }
        : {
            kind: 'terminated',
            reason: 'Error',
            exitCode: 1,
...
export const makeInitialWorld = (): WorldSnapshot => {
  const replicaSet: WorldEntity = {
    ...
    namespace: 'shop',
    status: 'ready',
    data: { specReplicas: 3, statusReplicas: 3, readyReplicas: 3 },
    title: text('api-rs'),
