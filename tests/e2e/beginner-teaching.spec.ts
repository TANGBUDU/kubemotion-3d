...
test('beginner journey stays problem-first and visually focused', async ({ page }) => {
  ...
  await page.screenshot({ path: `${reviewDir}/01-home-why-kubernetes.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${reviewDir}/02-one-container-is-enough.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${reviewDir}/03-three-copies-create-a-problem.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${reviewDir}/05-controller-restores-the-gap.png`, fullPage: false });

  await gotoBeginnerLesson(page, 'pending-and-scheduling', 1);
  const schedulerDiagnostics = await page.evaluate(() => window.__KUBEMOTION_TEST__?.getSceneDiagnostics());
  ...
  await page.screenshot({ path: `${reviewDir}/06-scheduler-one-control-line.png`, fullPage: false });
